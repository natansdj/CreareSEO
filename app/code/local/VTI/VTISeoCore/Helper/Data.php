...
class VTI_VTISeoCore_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    {
        $type = $product->getAttributeText('vtiseo_discontinued');
        ...
        if($type === '301 Redirect to Product SKU'){
            ...
            $sku = $product->getVTIseoDiscontinuedProduct();
            if($sku){
    ...
    public function saveFileContentToConfig($file, $field)
    {
        ...
        if ($io->fileExists($file))
        {
            try
            {
                ...
                Mage::getModel('core/config')->saveConfig('vti'.$field.'/files/'.$field, $contents);
